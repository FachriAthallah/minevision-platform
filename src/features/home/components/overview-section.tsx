    ...
    >
      ...
      <div className="mt-12 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
        {statistics.map((statistic, index) => {
          ...
          return (
            <Reveal direction="up" delay={index * 70} key={statistic.label}>
              <article className="flex h-full items-center gap-5 rounded-2xl border border-border bg-surface/70 p-6 shadow-sm backdrop-blur transition-colors hover:border-accent/40">
                <Icon
                  aria-hidden="true"
      ...
      <div className="mt-12 grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        {modules.map((item, index) => {
          ...
          return (
            <Reveal direction="up" delay={index * 70} key={item.title}>
              <article className="group relative flex h-full overflow-hidden rounded-2xl border border-brand-cyan/20 bg-[linear-gradient(135deg,rgba(40,103,228,0.19)_0%,rgba(0,177,196,0.12)_50%,rgba(60,195,171,0.15)_100%)] p-7 shadow-[0_18px_46px_rgba(0,0,0,0.18)] backdrop-blur transition-all hover:-translate-y-1 hover:border-brand-cyan/50 hover:shadow-[0_22px_55px_rgba(0,177,196,0.12)]">
