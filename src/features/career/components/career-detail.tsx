  ...
  return <Container className="max-w-[1320px] pb-20 pt-32 sm:pt-36">
    <Reveal direction="up">
      <header className="rounded-3xl border border-border bg-surface p-6 sm:p-9">
        <Link href="/career" className={cn(buttonVariants({ variant: "primary" }), "mb-7 max-w-full whitespace-normal motion-reduce:transition-none")}><ArrowLeft className="size-4 shrink-0" aria-hidden="true" />Kembali ke kategori karier</Link>
    ...
    <div className="mt-8 grid items-start gap-7 lg:grid-cols-[250px_minmax(0,1fr)] xl:gap-9">
      ...
      <article className="min-w-0 space-y-7">
        {paragraphs.length > 0 && <Reveal direction="up"><Card className="p-6 sm:p-8"><CareerSectionHeading title="Mengenal Bidang Ini" section="overview" /><div className="mt-5 space-y-4 leading-8 text-muted-foreground">{paragraphs.map((text, index) => <p key={index}>{text.replace(/^#+\s*/, "")}</p>)}</div></Card></Reveal>}
        ...
        {careerSections.map(({ key, label }, index) => <Reveal key={key} direction="up" delay={Math.min(index, 2) * 40}><CareerMaterialSection section={key} title={label} groups={careerMaterialGroups(profile.body, key, career.sections[key])} /></Reveal>)}
        ...
        <CareerCta />
        {educationSlug ? <Reveal direction="up" delay={40}><Link href={`/education/${educationSlug}`} className="flex items-center justify-between gap-4 rounded-2xl border border-brand-cyan/25 bg-brand-cyan/5 p-5 hover:bg-brand-cyan/10 focus-visible:outline-2 focus-visible:outline-brand-cyan"><span><span className="block text-xs font-bold uppercase tracking-wider text-brand-cyan">Materi terkait</span><span className="mt-2 block font-bold">Pelajari fondasi Education untuk bidang ini</span></span><ArrowRight aria-hidden="true" className="size-5 shrink-0" /></Link></Reveal> : null}
