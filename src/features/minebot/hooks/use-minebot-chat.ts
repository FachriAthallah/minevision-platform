...
export function useMineBotChat() {
  ...
  // Keep the conversation across page navigation and full reloads.
  // Persistence stays client-side in sessionStorage.
  useEffect(() => {
    persistMessages(messages);
  }, [messages]);

  const patchAssistant = useCallback(
    (id: string, fn: (m: MineBotUiMessage) => MineBotUiMessage) => {
  ...
  const sendMessage = useCallback(
    async (content: string, context?: MineBotRequestContext) => {
      ...
      const processEvent = (
        event: NonNullable<ReturnType<NDJSONParser["parseChunk"]>[number]>,
      ) => {
        if (event.type === "final" || event.type === "error") {
          hasTerminalEvent = true;
        }

        patchAssistant(assistantMessageId, (message) =>
  ...
  const retryLast = useCallback(() => {
    if (lastQuestionRef.current && !isSendingRef.current) {
      return sendMessage(
        ...
        lastQuestionRef.current.context,
      );
    }
