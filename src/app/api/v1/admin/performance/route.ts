...
export async function GET(request: NextRequest) {
  ...
  try {
    const vitals = await getVitalsMetrics(range);
    ...
    console.error("Admin performance failed.", caught instanceof Error ? caught.message : String(caught));
    return adminJsonError("INTERNAL_SERVER_ERROR", "Performance belum dapat dimuat.", 500);
  }
}