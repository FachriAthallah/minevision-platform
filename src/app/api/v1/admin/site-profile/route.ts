...
import { siteProfileSchema } from "@/features/admin/schemas/admin-validation";

export const runtime = "nodejs";
export const dynamic = "force-dynamic";

const patchBodySchema = z.object({
  data: siteProfileSchema,
...
export async function GET() {
  ...
  if (!guard.access.canManageConfig) {
    return adminJsonError("ADMIN_FORBIDDEN", "Role aktif tidak dapat mengelola site profile.", 403);
  }

  const row = await getSettingAdmin(SITE_SETTING_KEYS.siteProfile);
...
export async function PATCH(request: NextRequest) {
  ...
  if (!parsed.success) {
    return adminJsonError("INVALID_BODY", "Data site profile tidak valid.", 400);
  }
