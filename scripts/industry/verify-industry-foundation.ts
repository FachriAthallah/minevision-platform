...
type SummaryRow = {
  ...
  site_count: number;
  coordinate_site_count: number;
  approximate_site_count: number;
...
) as coordinate_site_count,
(
  select count(*)::integer
  from public.industry_operation_sites
  where coordinate_precision = 'approximate'
) as approximate_site_count,
  ...
  assertCondition(
    summary.source_count === 12,
    `Sumber harus 12, ditemukan ${summary.source_count}.`,
  );
  assertCondition(
    summary.production_count === 75,
    `Produksi harus 75, ditemukan ${summary.production_count}.`,
  );
  assertCondition(
    summary.reported_production_count === 69,
    `Produksi reported harus 69, ditemukan ${summary.reported_production_count}.`,
  );
  assertCondition(
    summary.not_normalized_count === 4,
    `Produksi not_normalized harus 4, ditemukan ${summary.not_normalized_count}.`,
  );
  assertCondition(
    summary.not_reported_count === 2,
    `Produksi not_reported harus 2, ditemukan ${summary.not_reported_count}.`,
  );
  assertCondition(
    summary.financial_count === 108,
    `Keuangan harus 108, ditemukan ${summary.financial_count}.`,
  );
  assertCondition(
    summary.usd_financial_count === 72,
    `Keuangan USD harus 72, ditemukan ${summary.usd_financial_count}.`,
  );
  assertCondition(
    summary.idr_financial_count === 36,
    `Keuangan IDR harus 36, ditemukan ${summary.idr_financial_count}.`,
  );
  assertCondition(
    summary.site_count === 47,
    `Lokasi harus 47, ditemukan ${summary.site_count}.`,
  );

  assertCondition(
    summary.coordinate_site_count === 46,
    `Lokasi berkoordinat harus 46, ditemukan ${summary.coordinate_site_count}.`,
  );

  assertCondition(
    summary.approximate_site_count === 46,
    `Koordinat approximate harus 46, ditemukan ${summary.approximate_site_count}.`,
  );

  assertCondition(
    summary.draft_site_count === 1,
    `Lokasi draft/pending harus 1, ditemukan ${summary.draft_site_count}.`,
  );

  assertCondition(
    summary.published_site_count === 46,
    `Lokasi published harus 46, ditemukan ${summary.published_site_count}.`,
  );

  console.log(`[OK] sumber: ${summary.source_count}`);
  console.log(
    `[OK] produksi: ${summary.production_count} (${summary.reported_production_count} reported, ${summary.not_normalized_count} not_normalized, ${summary.not_reported_count} not_reported)`,
  );
  console.log(
    `[OK] keuangan: ${summary.financial_count} (${summary.usd_financial_count} USD, ${summary.idr_financial_count} IDR)`,
  );
  console.log(
    `[OK] lokasi operasi: ${summary.site_count} total ` +
      `(${summary.published_site_count} published, ` +
      `${summary.draft_site_count} pending/draft)`,
  );
  ...
  assertCondition(
    invalidProduction === 0,
    `Ditemukan ${invalidProduction} produksi dengan normalisasi/status tidak valid.`,
  );
  ...
  assertCondition(
    invalidFinancials === 0,
    `Ditemukan ${invalidFinancials} keuangan dengan normalisasi/status tidak valid.`,
  );

  const invalidCompanyYearFinancials = await singleInvalidCount(sqlClient<
    InvalidRow[]
  >`
    select count(*)::integer as invalid_count
    from (
  ...
  `);
  assertCondition(
    invalidCompanyYearFinancials === 0,
    `Ditemukan ${invalidCompanyYearFinancials} pasangan perusahaan-tahun tanpa tiga metrik keuangan.`,
  );

  const invalidSites = await singleInvalidCount(
    sqlClient<InvalidRow[]>`
    select count(*)::integer as invalid_count
    ...
    where not (
      (
        ...
        and site.coordinate_precision = 'approximate'
        and site.is_active = true
        and site.verification_status = 'verified'
        and site.publication_status = 'published'
      )
      or
      (
        company.slug = 'alamtri-resources'
        and site.slug = 'juloi-kalteng-sumber-barito'
        and site.latitude is null
        and site.longitude is null
        and site.coordinate_precision is null
        and site.verification_status = 'pending'
        and site.publication_status = 'draft'
      )
    )
  `,
  );
  ...
  assertCondition(
    invalidSites === 0,
    `Ditemukan ${invalidSites} lokasi dengan koordinat atau status publikasi tidak valid.`,
  );

  const timahLoss = (
    await sqlClient<ValueRow[]>`
  ...
  assertCondition(
    Number(timahLoss) === -449_670_000_000,
    `Rugi TIMAH 2023 tidak sesuai: ${timahLoss}.`,
  );
  ...
  assertCondition(
    harumCoal === 0,
    "Seri batubara Harum belum dinormalisasi sebagai ton penuh.",
  );
  ...
  assertCondition(
    freeportEarnings === 0,
    "Laba operasi Freeport tidak boleh dilabeli laba bersih/tahun berjalan.",
  );

  console.log("[OK] normalisasi nilai produksi dan keuangan konsisten");
  console.log(
    "[OK] koreksi Harum, TIMAH, Freeport, dan data kosong Harita terkunci",
  );
  console.log("[OK] RLS situs hanya menampilkan 46 marker verified/published");
